...
class SnapPage(QWidget):
    def __init__(self):
        ...
        self.query.setPlaceholderText("Snap name or search term")
    ...
    def search(self):
        ...
        if not q:
            return
        self._append_result("Snap search", self.service.search(q))
    ...
    def install(self):
        ...
        if QMessageBox.question(self, "Install", f"Install {q}?") != QMessageBox.StandardButton.Yes:
            return
        self._append_result("Snap install", self.service.install(q))
    ...
    def remove(self):
        ...
        if QMessageBox.question(self, "Remove", f"Remove {q}?") != QMessageBox.StandardButton.Yes:
            return
        self._append_result("Snap remove", self.service.remove(q))

    def list_installed(self):
        rows = self.service.list_installed()
        self.output.setPlainText("Installed snaps:\n" + "\n".join(rows))
